...
static int handle_test_link_video_pattern(struct dptx *dptx, int stream)
{
	...
	if (ret_dpcd < 0)
		return ret_dpcd;
	ret_dpcd = drm_dp_dpcd_readb(&dptx->aux_dev, DP_TEST_MISC0, &misc);
	if (ret_dpcd < 0)
		return ret_dpcd;
	...
	switch (ycbcr_coeff) {
	case DP_YCBCR_COEFFICIENTS_ITU601:
		DRM_DEBUG("DP_YCBCR_COEFFICIENTS_ITU601\n");
		ycbcr_coeff_map = ITU601;
		break;
	case DP_YCBCR_COEFFICIENTS_ITU709:
		DRM_DEBUG("DP_YCBCR_COEFFICIENTS_ITU709:\n");
	...
	case DP_COLOR_FORMAT_RGB:
		DRM_DEBUG("DP_COLOR_FORMAT_RGB\n");
		color_format_map = RGB;
		break;
	case DP_COLOR_FORMAT_YCbCr422:
		DRM_DEBUG("DP_COLOR_FORMAT_YCbCr422\n");
		color_format_map = YCBCR422;
		break;
	case DP_COLOR_FORMAT_YCbCr444:
		DRM_DEBUG("DP_COLOR_FORMAT_YCbCr444\n");
