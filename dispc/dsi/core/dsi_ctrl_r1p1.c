...
{
	struct dsi_reg *reg = (struct dsi_reg *)ctx->base;
	union _0x120 dpi_vfp;

	dpi_vfp.val = readl(&reg->DPI_VFP);
	dpi_vfp.bits.vfp_lines = lines;

	writel(dpi_vfp.val, &reg->DPI_VFP);
}
/**
...
const struct dsi_core_ops dsi_ctrl_r1p1_ops = {
