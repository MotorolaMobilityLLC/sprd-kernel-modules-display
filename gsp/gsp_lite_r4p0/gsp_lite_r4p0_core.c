...
#include <drm/gsp_lite_r4p0_cfg.h>
#include "../gsp_core.h"
#include "../gsp_kcfg.h"
#include "../gsp_debug.h"
#include "../gsp_dev.h"
...
#include "../gsp_layer.h"
#include "gsp_lite_r4p0_core.h"
#include "gsp_lite_r4p0_reg.h"
#include "gsp_coef_cal.h"
#include "../gsp_interface.h"

static int zorder_used[LITE_R4P0_IMGL_NUM + LITE_R4P0_OSDL_NUM] = {0};
int gsp_lite_r4p0_layer_num;//gsp_enabled_layer_count->gsp_lite_r4p0_layer_num;

static void print_image_layer_cfg(struct gsp_lite_r4p0_img_layer *layer)
{
	struct gsp_lite_r4p0_img_layer_params *params = NULL;
...
static void print_osd_layer_cfg(struct gsp_lite_r4p0_osd_layer *layer)
{
	struct gsp_lite_r4p0_osd_layer_params *params = NULL;
...
static void print_des_layer_cfg(struct gsp_lite_r4p0_des_layer *layer)
{
	struct gsp_lite_r4p0_des_layer_params *params = NULL;
...
static void gsp_lite_r4p0_core_cfg_print(struct gsp_lite_r4p0_cfg *cfg)
{
	int icnt = 0;
...
void gsp_lite_r4p0_core_dump(struct gsp_core *c)
{
	int icnt = 0;
	struct LITE_R4P0_GSP_CTL_REG_T reg_struct;

	memset(&reg_struct, 0, sizeof(struct LITE_R4P0_GSP_CTL_REG_T));

	reg_struct.glb_cfg.value = gsp_core_reg_read(LITE_R4P0_GSP_GLB_CFG(c->base));
	reg_struct.mod_cfg.value = gsp_core_reg_read(LITE_R4P0_GSP_MOD_CFG(c->base));
	reg_struct.secure_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_SECURE_CFG(c->base));

	reg_struct.des_data_cfg.value =
		gsp_core_reg_read(LITE_R4P0_DES_DATA_CFG(c->base));
	reg_struct.des_y_addr.value =
		gsp_core_reg_read(LITE_R4P0_DES_Y_ADDR(c->base));
	reg_struct.des_u_addr.value =
		gsp_core_reg_read(LITE_R4P0_DES_U_ADDR(c->base));
	reg_struct.des_v_addr.value =
		gsp_core_reg_read(LITE_R4P0_DES_V_ADDR(c->base));
	reg_struct.des_pitch.value =
		gsp_core_reg_read(LITE_R4P0_DES_PITCH(c->base));
	reg_struct.back_rgb.value =
		gsp_core_reg_read(LITE_R4P0_BACK_RGB(c->base));
	reg_struct.work_area_size.value =
		gsp_core_reg_read(LITE_R4P0_WORK_AREA_SIZE(c->base));
	reg_struct.work_area_xy.value =
		gsp_core_reg_read(LITE_R4P0_WORK_AREA_XY(c->base));

	for (icnt = 0; icnt < LITE_R4P0_IMGL_NUM; icnt++) {
		reg_struct.limg_cfg[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_CFG(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_y_addr[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_Y_ADDR(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_u_addr[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_U_ADDR(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_v_addr[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_V_ADDR(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_pitch[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_PITCH(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_clip_start[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_CLIP_START(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_clip_size[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_CLIP_SIZE(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_des_start[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_DES_START(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_pallet_rgb[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_PALLET_RGB(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_ck[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_CK(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.y2y_y_param[icnt].value =
			gsp_core_reg_read(LITE_R4P0_Y2Y_Y_PARAM(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.y2y_u_param[icnt].value =
			gsp_core_reg_read(LITE_R4P0_Y2Y_U_PARAM(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.y2y_v_param[icnt].value =
			gsp_core_reg_read(LITE_R4P0_Y2Y_V_PARAM(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
		reg_struct.limg_des_scl_size[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LIMG_DES_SIZE(
				(c->base + icnt * LITE_R4P0_LIMG_OFFSET)));
	}

	for (icnt = 0; icnt < LITE_R4P0_OSDL_NUM; icnt++) {
		reg_struct.losd_cfg[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_CFG(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
		reg_struct.losd_r_addr[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_R_ADDR(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
		reg_struct.losd_pitch[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_PITCH(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
		reg_struct.losd_clip_start[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_CLIP_START(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
		reg_struct.losd_clip_size[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_CLIP_SIZE(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
		reg_struct.losd_des_start[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_DES_START(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
		reg_struct.losd_pallet_rgb[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_PALLET_RGB(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
		reg_struct.losd_ck[icnt].value =
			gsp_core_reg_read(LITE_R4P0_LOSD_CK(
				(c->base + icnt * LITE_R4P0_LOSD_OFFSET)));
	}

	reg_struct.ip_rev.value =
		gsp_core_reg_read(LITE_R4P0_GSP_IP_REV(c->base));
	reg_struct.debug_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG_CFG(c->base));
	reg_struct.debug1_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG1(c->base));
	reg_struct.debug2_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG2(c->base));
	reg_struct.debug3_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG3(c->base));
	reg_struct.debug4_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG4(c->base));
		reg_struct.debug5_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG5(c->base));
	reg_struct.debug6_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG6(c->base));
	reg_struct.debug7_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG7(c->base));
	reg_struct.debug8_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG8(c->base));
	reg_struct.debug9_cfg.value =
		gsp_core_reg_read(LITE_R4P0_GSP_DEBUG9(c->base));
...
static void gsp_lite_r4p0_core_cfg_reinit(struct gsp_lite_r4p0_cfg *cfg)
{
	...
	GSP_DEBUG("gsp_lite_r4p0 cfg parent kcfg[%d]\n", gsp_kcfg_to_tag(kcfg));
	if (cfg->common.init != 1) {
		GSP_ERR("gsp_lite_r4p0 cfg[%d] has not been initialized\n",
			cfg->common.tag);
		return;
	}
	...
	for (icnt = 0; icnt < LITE_R4P0_IMGL_NUM; icnt++)
		memset(&cfg->limg[icnt].params, 0,
			sizeof(struct gsp_lite_r4p0_img_layer_params));
	for (icnt = 0; icnt < LITE_R4P0_OSDL_NUM; icnt++)
		memset(&cfg->losd[icnt].params, 0,
			sizeof(struct gsp_lite_r4p0_osd_layer_params));
	memset(&cfg->ld1.params, 0, sizeof(struct gsp_lite_r4p0_des_layer_params));
}

static int gsp_lite_r4p0_core_capa_init(struct gsp_core *core)
{
	struct gsp_lite_r4p0_capability *capa = NULL;
	struct gsp_rect max = {0, 0, 8191, 8191};
	struct gsp_rect min = {0, 0, 4, 4};

	capa = (struct gsp_lite_r4p0_capability *)core->capa;

	/* common information initialize */
	capa->common.magic = GSP_CAPABILITY_MAGIC;
	strcpy(capa->common.version, "LITE_R4P0");
	capa->common.capa_size = sizeof(struct gsp_lite_r4p0_capability);

	capa->common.max_layer = LITE_R4P0_IMGL_NUM + LITE_R4P0_OSDL_NUM;
	capa->common.max_img_layer = LITE_R4P0_IMGL_NUM;
...
static void gsp_lite_r4p0_int_clear_and_disable(struct gsp_core *core)
{
	struct LITE_R4P0_GSP_INT_REG gsp_int_value;
	struct LITE_R4P0_GSP_INT_REG gsp_int_mask;

	if (core == NULL) {
		GSP_ERR("lite_r4p0 interrupt clear with null core\n");
		return;
	}
	...
	gsp_core_reg_update(LITE_R4P0_GSP_INT(core->base),
			gsp_int_value.value, gsp_int_mask.value);

}

static void gsp_lite_r4p0_int_clear(struct gsp_core *core)
{
	struct LITE_R4P0_GSP_INT_REG gsp_int_value;
	struct LITE_R4P0_GSP_INT_REG gsp_int_mask;
...
static void gsp_lite_r4p0_coef_cache_init(struct gsp_lite_r4p0_core *core)
{
	uint32_t i = 0;
	...
	if (core->cache_coef_init_flag == 0) {
		...
		while (i < LITE_R4P0_GSP_COEF_CACHE_MAX) {
...
static void gsp_lite_r4p0_core_cfg_init(struct gsp_lite_r4p0_cfg *cfg,
					struct gsp_kcfg *kcfg)
{
	...
	cfg->common.layer_num = LITE_R4P0_IMGL_NUM + LITE_R4P0_OSDL_NUM;
	cfg->common.init = 1;
...
int gsp_lite_r4p0_core_init(struct gsp_core *core)
{
	int ret = 0;
	struct gsp_kcfg *kcfg = NULL;
	struct gsp_lite_r4p0_core *c = (struct gsp_lite_r4p0_core *)core;
	...
	gsp_lite_r4p0_core_capa_init(core);

	gsp_lite_r4p0_coef_cache_init(c);

	/* to do some specific initialize operations here */
	list_for_each_entry(kcfg, &core->kcfgs, sibling) {
		gsp_lite_r4p0_core_cfg_init(
			(struct gsp_lite_r4p0_cfg *)kcfg->cfg, kcfg);
	}

	return ret;
}

int gsp_lite_r4p0_core_alloc(struct gsp_core **core, struct device_node *node)
{
	void *capa = NULL;
	struct platform_device *pdev = NULL;

	if (IS_ERR_OR_NULL(core)) {
		GSP_ERR("lite_r4p0 core alloc params error\n");
		return -1;
	}
	...
	if (IS_ERR_OR_NULL(pdev)) {
		GSP_ERR("find gsp_lite_r4p0 device by child node failed\n");
		return -1;
	}
	*core = devm_kzalloc(&pdev->dev, sizeof(struct gsp_lite_r4p0_core),
						GFP_KERNEL);
	if (IS_ERR_OR_NULL(*core)) {
	...
	memset(*core, 0, sizeof(struct gsp_lite_r4p0_core));
	(*core)->cfg_size = sizeof(struct gsp_lite_r4p0_cfg);
	(*core)->dev = &pdev->dev;
	if (dev_name(&pdev->dev))
		GSP_INFO("core[%d] device name: %s\n", (*core)->id,
			 dev_name(&pdev->dev));

	capa = kzalloc(sizeof(struct gsp_lite_r4p0_capability), GFP_KERNEL);
	if (IS_ERR_OR_NULL(capa)) {
		GSP_ERR("gsp_lite_r4p0 capability memory allocated failed\n");
		devm_kfree(&pdev->dev, *core);
		return -1;
	}
	(*core)->capa = capa;
	(*core)->capa->capa_size = sizeof(struct gsp_lite_r4p0_capability);
	(*core)->node = node;
	GSP_ERR("gsp_lite_r4p0_cap_size = %d\n", (int)(*core)->capa->capa_size);
	GSP_INFO("gsp_lite_r4p0 core allocate success\n");
	return 0;
}

int gsp_lite_r4p0_core_devset(struct device *drm_gsp[GSP_MAX_NUM], struct device *gspdev)
{
	GSP_ERR("gsp_lite_r4p0_core_devset 1");
	drm_gsp[1] = gspdev;
	return 0;
}

static void gsp_lite_r4p0_core_irq_enable(struct gsp_core *core)
{
	struct LITE_R4P0_GSP_INT_REG gsp_int_value;
	struct LITE_R4P0_GSP_INT_REG gsp_int_mask;
...
static irqreturn_t gsp_lite_r4p0_core_irq_handler(int irq, void *data)
{
	...
	gsp_int_value.value =
		gsp_core_reg_read(LITE_R4P0_GSP_INT(core->base));
	...
	if (gsp_int_value.INT_FBCDPL_RAW || gsp_int_value.INT_FBCDHD_RAW) {
		...
		gsp_lite_r4p0_int_clear(core);

		return IRQ_HANDLED;
	}

	gsp_lite_r4p0_int_clear_and_disable(core);
...
int gsp_lite_r4p0_core_enable(struct gsp_core *c)
{
	int ret = -1;
	struct gsp_lite_r4p0_core *core = NULL;

	core = (struct gsp_lite_r4p0_core *)c;
	...
	ret = clk_prepare_enable(core->gsp1_eb);
	if (ret) {
		GSP_ERR("enable gsp1_eb fail\n");
		goto gsp_eb_unprepare;
	}

	clk_set_parent(core->gsp1_clk, NULL);
	ret = clk_set_parent(core->gsp1_clk, core->gsp1_clk_parent);
	if (ret) {
		GSP_ERR("select gsp1 clk source failed !\n");
		goto exit;
	}

	ret = clk_prepare_enable(core->gsp1_clk);
	if (ret) {
		GSP_ERR("enable gsp1_clk failed !\n");
		goto gsp_clk_unprepare;
	}


	gsp_lite_r4p0_int_clear(c);
	gsp_lite_r4p0_core_irq_enable(c);

	sprd_iommu_restore(c->dev);
	goto exit;

gsp_clk_unprepare:
	clk_disable_unprepare(core->gsp1_clk);
gsp_eb_unprepare:
	clk_disable_unprepare(core->gsp1_eb);
exit:
	return ret;
}

void gsp_lite_r4p0_core_disable(struct gsp_core *c)
{
	struct gsp_lite_r4p0_core *core = NULL;

	core = (struct gsp_lite_r4p0_core *)c;
	gsp_lite_r4p0_int_clear_and_disable(c);
	clk_disable_unprepare(core->gsp1_clk);
	clk_disable_unprepare(core->gsp1_eb);
	clk_disable_unprepare(core->gsp_dpuvsp_eb);
}

static int gsp_lite_r4p0_core_parse_clk(struct gsp_lite_r4p0_core *core)
{
	int status = 0;
	...
	core->gsp1_eb = of_clk_get_by_name(core->common.node,
			"clk_gsp1_eb");

	core->gsp1_clk = of_clk_get_by_name(core->common.node,
			"clk_gsp1");

	core->gsp1_clk_parent = of_clk_get_by_name(core->common.node,
			"clk_src_512m");

	if (IS_ERR_OR_NULL(core->gsp1_eb)
			|| IS_ERR_OR_NULL(core->gsp_dpuvsp_eb)
			|| IS_ERR_OR_NULL(core->gsp1_clk)
			|| IS_ERR_OR_NULL(core->gsp1_clk_parent)) {
		GSP_ERR("parse gsp1 clk failed\n");
		status = -1;
	}
...
static int gsp_lite_r4p0_core_parse_irq(struct gsp_core *core)
{
	int ret = -1;
	struct device *dev = NULL;
	static const char * const lite_r4p0_irq_name[] = {
		"GSP0", "GSP1"
	};
	...
	if (core->irq)
		ret = devm_request_irq(dev, core->irq,
				gsp_lite_r4p0_core_irq_handler,
				IRQF_SHARED, lite_r4p0_irq_name[core->id], core);

	if (ret)
		GSP_ERR("lite_r4p0 core[%d] request irq failed! %d\n",
		core->id, core->irq);

	return ret;
}

int gsp_lite_r4p0_core_parse_dt(struct gsp_core *core)
{
	int ret = -1;
	struct device *dev = NULL;
	struct gsp_lite_r4p0_core *lite_r4p0_core = NULL;

	dev = container_of(&core->node, struct device, of_node);
	lite_r4p0_core = (struct gsp_lite_r4p0_core *)core;

	lite_r4p0_core->gsp_ctl_reg_base = core->base;
	GSP_INFO("gsp_ctl_reg_base: 0x%p\n", core->base);

	ret = gsp_lite_r4p0_core_parse_irq(core);
	if (ret) {
		GSP_ERR("core[%d] parse irq failed\n", core->id);
		return ret;
	}

	gsp_lite_r4p0_core_parse_clk(lite_r4p0_core);

	return ret;
}


static void gsp_lite_r4p0_core_misc_reg_set(struct gsp_core *core,
			struct gsp_lite_r4p0_cfg *cfg)
{
	void __iomem *base = NULL;
	struct LITE_R4P0_WORK_AREA_XY_REG  work_area_xy_value;
	struct LITE_R4P0_WORK_AREA_XY_REG work_area_xy_mask;
	struct LITE_R4P0_WORK_AREA_SIZE_REG work_area_size_value;
	struct LITE_R4P0_WORK_AREA_SIZE_REG work_area_size_mask;
	struct LITE_R4P0_GSP_MOD_CFG_REG gsp_mod_cfg_value;
	struct LITE_R4P0_GSP_MOD_CFG_REG gsp_mod_cfg_mask;
	...
	gsp_core_reg_update(LITE_R4P0_GSP_MOD_CFG(base),
	...
	gsp_core_reg_update(LITE_R4P0_WORK_AREA_XY(base),
	...
	gsp_core_reg_update(LITE_R4P0_WORK_AREA_SIZE(base),
		work_area_size_value.value, work_area_size_mask.value);
}

static void gsp_lite_r4p0_core_limg_reg_set(void __iomem *base,
			   struct gsp_lite_r4p0_img_layer *layer, int layer_index)
{
	struct LITE_R4P0_LAYERIMG_CFG_REG limg_cfg_value;
	struct LITE_R4P0_LAYERIMG_CFG_REG limg_cfg_mask;
	struct LITE_R4P0_LAYERIMG_Y_ADDR_REG limg_y_addr_value;
	struct LITE_R4P0_LAYERIMG_Y_ADDR_REG limg_y_addr_mask;
	struct LITE_R4P0_LAYERIMG_U_ADDR_REG limg_u_addr_value;
	struct LITE_R4P0_LAYERIMG_U_ADDR_REG limg_u_addr_mask;
	struct LITE_R4P0_LAYERIMG_V_ADDR_REG limg_v_addr_value;
	struct LITE_R4P0_LAYERIMG_V_ADDR_REG limg_v_addr_mask;
	struct LITE_R4P0_LAYERIMG_PITCH_REG limg_pitch_value;
	struct LITE_R4P0_LAYERIMG_PITCH_REG limg_pitch_mask;
	struct LITE_R4P0_LAYERIMG_CLIP_SIZE_REG limg_clip_size_value;
	struct LITE_R4P0_LAYERIMG_CLIP_SIZE_REG limg_clip_size_mask;
	struct LITE_R4P0_LAYERIMG_CLIP_START_REG limg_clip_start_value;
	struct LITE_R4P0_LAYERIMG_CLIP_START_REG limg_clip_start_mask;
	struct LITE_R4P0_LAYERIMG_DES_START_REG limg_des_start_value;
	struct LITE_R4P0_LAYERIMG_DES_START_REG limg_des_start_mask;
	struct LITE_R4P0_LAYERIMG_CK_REG limg_ck_value;
	struct LITE_R4P0_LAYERIMG_CK_REG limg_ck_mask;
	struct LITE_R4P0_LAYERIMG_DES_SCL_SIZE_REG limg_des_size_value;
	struct LITE_R4P0_LAYERIMG_DES_SCL_SIZE_REG limg_des_size_mask;
	struct gsp_lite_r4p0_img_layer_params *limg_params = NULL;
	int i = 0;
	...
	if (layer->common.enable != 1) {
		...
		for (i = 0; i < LITE_R4P0_IMGL_NUM; i++) {
			if (!zorder_used[i]) {
		...
		gsp_core_reg_update(LITE_R4P0_LIMG_CFG(
			(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_Y_ADDR(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_U_ADDR(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_V_ADDR(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_PITCH(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_CLIP_SIZE(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_CLIP_START(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_DES_START(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_DES_SIZE(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
		limg_des_size_value.value, limg_des_size_mask.value);

	/* limg pallet rgb set */
	if (limg_params->pallet_en) {
		struct LITE_R4P0_LAYERIMG_PALLET_RGB_REG limg_pallet_value;
		struct LITE_R4P0_LAYERIMG_PALLET_RGB_REG limg_pallet_mask;
		...
		gsp_core_reg_update(LITE_R4P0_LIMG_PALLET_RGB(
			(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LIMG_CK(
		(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
	...
	if (limg_params->y2y_mod == 0) {
		struct LITE_R4P0_Y2Y_Y_PARAM_REG y2y_y_para_value;
		struct LITE_R4P0_Y2Y_Y_PARAM_REG y2y_y_para_mask;
		struct LITE_R4P0_Y2Y_U_PARAM_REG y2y_u_para_value;
		struct LITE_R4P0_Y2Y_U_PARAM_REG y2y_u_para_mask;
		struct LITE_R4P0_Y2Y_V_PARAM_REG y2y_v_para_value;
		struct LITE_R4P0_Y2Y_V_PARAM_REG y2y_v_para_mask;
		...
		gsp_core_reg_update(LITE_R4P0_Y2Y_Y_PARAM(
			(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
		...
		gsp_core_reg_update(LITE_R4P0_Y2Y_U_PARAM(
			(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
		...
		gsp_core_reg_update(LITE_R4P0_Y2Y_V_PARAM(
			(base + layer_index * LITE_R4P0_LIMG_OFFSET)),
...
static void gsp_lite_r4p0_core_losd_reg_set(void __iomem *base,
			   struct gsp_lite_r4p0_osd_layer *layer, int layer_index)
{
	struct LITE_R4P0_LAYEROSD_CFG_REG losd_cfg_value;
	struct LITE_R4P0_LAYEROSD_CFG_REG losd_cfg_mask;
	struct LITE_R4P0_LAYEROSD_R_ADDR_REG losd_r_addr_value;
	struct LITE_R4P0_LAYEROSD_R_ADDR_REG losd_r_addr_mask;
	struct LITE_R4P0_LAYEROSD_PITCH_REG losd_pitch_value;
	struct LITE_R4P0_LAYEROSD_PITCH_REG losd_pitch_mask;
	struct LITE_R4P0_LAYEROSD_CLIP_SIZE_REG losd_clip_size_value;
	struct LITE_R4P0_LAYEROSD_CLIP_SIZE_REG losd_clip_size_mask;
	struct LITE_R4P0_LAYEROSD_CLIP_START_REG losd_clip_start_value;
	struct LITE_R4P0_LAYEROSD_CLIP_START_REG losd_clip_start_mask;
	struct LITE_R4P0_LAYEROSD_DES_START_REG losd_des_start_value;
	struct LITE_R4P0_LAYEROSD_DES_START_REG losd_des_start_mask;
	struct LITE_R4P0_LAYEROSD_CK_REG losd_ck_value;
	struct LITE_R4P0_LAYEROSD_CK_REG losd_ck_mask;
	struct gsp_lite_r4p0_osd_layer_params *losd_params = NULL;
	int i = 0;
	...
	if (layer->common.enable != 1) {
		...
		for (i = LITE_R4P0_IMGL_NUM; i < LITE_R4P0_IMGL_NUM + LITE_R4P0_OSDL_NUM; i++) {
			if (!zorder_used[i]) {
		...
		gsp_core_reg_update(LITE_R4P0_LOSD_CFG(
			(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LOSD_R_ADDR(
		(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LOSD_PITCH(
		(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LOSD_CLIP_START(
		(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LOSD_CLIP_SIZE(
		(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LOSD_DES_START(
		(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
		losd_des_start_value.value, losd_des_start_mask.value);

	/* losd pallet set */
	if (losd_params->pallet_en) {
		struct LITE_R4P0_LAYEROSD_PALLET_RGB_REG losd_pallet_value;
		struct LITE_R4P0_LAYEROSD_PALLET_RGB_REG losd_pallet_mask;
		...
		gsp_core_reg_update(LITE_R4P0_LOSD_PALLET_RGB(
			(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(LITE_R4P0_LOSD_CK(
		(base + layer_index * LITE_R4P0_LOSD_OFFSET)),
...
static void gsp_lite_r4p0_core_ld1_reg_set(void __iomem *base,
			   struct gsp_lite_r4p0_des_layer *layer)
{
	struct LITE_R4P0_DES_DATA_CFG_REG des_cfg_value;
	struct LITE_R4P0_DES_DATA_CFG_REG des_cfg_mask;
	struct LITE_R4P0_DES_Y_ADDR_REG des_y_addr_value;
	struct LITE_R4P0_DES_Y_ADDR_REG des_y_addr_mask;
	struct LITE_R4P0_DES_U_ADDR_REG des_u_addr_value;
	struct LITE_R4P0_DES_U_ADDR_REG des_u_addr_mask;
	struct LITE_R4P0_DES_V_ADDR_REG des_v_addr_value;
	struct LITE_R4P0_DES_V_ADDR_REG des_v_addr_mask;
	struct LITE_R4P0_DES_PITCH_REG des_pitch_value;
	struct LITE_R4P0_DES_PITCH_REG des_pitch_mask;
	struct gsp_lite_r4p0_des_layer_params *ld1_params = NULL;
	...
	gsp_core_reg_update(LITE_R4P0_DES_Y_ADDR(base),
	...
	gsp_core_reg_update(LITE_R4P0_DES_U_ADDR(base),
	...
	gsp_core_reg_update(LITE_R4P0_DES_V_ADDR(base),
	...
	gsp_core_reg_update(LITE_R4P0_DES_PITCH(base),
		des_pitch_value.value, des_pitch_mask.value);

	if (ld1_params->bk_para.bk_enable) {
		struct LITE_R4P0_BACK_RGB_REG back_rgb_value;
		struct LITE_R4P0_BACK_RGB_REG back_rgb_mask;
		...
		gsp_core_reg_update(LITE_R4P0_BACK_RGB(base),
	...
	gsp_core_reg_update(LITE_R4P0_DES_DATA_CFG(base),
		des_cfg_value.value, des_cfg_mask.value);
}

static void gsp_lite_r4p0_coef_gen_and_cfg(struct gsp_lite_r4p0_core *core,
						struct gsp_lite_r4p0_cfg *cmd)
{
	int i = 0;
	int jcnt = 0;
	...
	for (jcnt = 0; jcnt < LITE_R4P0_IMGL_NUM; jcnt++)
		if (cmd->limg[jcnt].params.scaling_en) {
			...
			ret_coef = gsp_lite_r4p0_gen_block_scaler_coef(core,
								src_w, src_h,
								dst_w, dst_h,
								hor_tap,
								ver_tap);
			...
			if (ret_coef) {
				for (i = 0; i < 128; i++)
					gsp_core_reg_update(
						LITE_R4P0_SCALE_COEF_ADDR(
						(core->gsp_ctl_reg_base +
						jcnt * LITE_R4P0_SCALE_COEF_OFFSET
						+ i * 4)),
						*ret_coef++, 0xffffffff);
...
static void gsp_lite_r4p0_core_run(struct gsp_core *core)
{
	struct LITE_R4P0_GSP_GLB_CFG_REG gsp_mod1_cfg_value;
	struct LITE_R4P0_GSP_GLB_CFG_REG gsp_mod1_cfg_mask;

	gsp_mod1_cfg_value.value = 0;
	gsp_mod1_cfg_value.GSP_RUN0 = 1;
	gsp_mod1_cfg_mask.value = 0;
	gsp_mod1_cfg_mask.GSP_RUN0 = 0x1;
	gsp_core_reg_update(LITE_R4P0_GSP_GLB_CFG(core->base),
		gsp_mod1_cfg_value.value, gsp_mod1_cfg_mask.value);
}

static int gsp_lite_r4p0_core_run_precheck(struct gsp_core *c)
{
	return 0;
}
int gsp_lite_r4p0_core_trigger(struct gsp_core *c)
{
	int ret = -1;
	int icnt = 0;
	void __iomem *base = NULL;
	struct gsp_kcfg *kcfg = NULL;
	struct gsp_lite_r4p0_cfg *cfg = NULL;
	struct gsp_lite_r4p0_core *core = (struct gsp_lite_r4p0_core *)c;
	struct LITE_R4P0_GSP_GLB_CFG_REG gsp_mod1_cfg_value;

	if (gsp_core_verify(c)) {
		GSP_ERR("gsp_lite_r4p0 core trigger params error\n");
		return ret;
	}

	kcfg = c->current_kcfg;
	if (gsp_kcfg_verify(kcfg)) {
		GSP_ERR("gsp_lite_r4p0 trigger invalidate kcfg\n");
		return ret;
	}

	/* hardware status check */
	gsp_mod1_cfg_value.value =
		gsp_core_reg_read(LITE_R4P0_GSP_GLB_CFG(c->base));
	...
	base = c->base;
	cfg = (struct gsp_lite_r4p0_cfg *)kcfg->cfg;

	gsp_lite_r4p0_coef_gen_and_cfg(core, cfg);

	gsp_lite_r4p0_core_misc_reg_set(c, cfg);
	for (icnt = 0; icnt < LITE_R4P0_IMGL_NUM; icnt++)
		gsp_lite_r4p0_core_limg_reg_set(base, &cfg->limg[icnt], icnt);
	for (icnt = 0; icnt < LITE_R4P0_OSDL_NUM; icnt++)
		gsp_lite_r4p0_core_losd_reg_set(base, &cfg->losd[icnt], icnt);
	gsp_lite_r4p0_core_ld1_reg_set(base, &cfg->ld1);

	if (gsp_lite_r4p0_core_run_precheck(c)) {
		GSP_ERR("lite_r4p0 core run precheck fail !\n");
		return GSP_K_CLK_CHK_ERR;
	}

	gsp_lite_r4p0_core_run(c);

	return 0;
};

int gsp_lite_r4p0_core_release(struct gsp_core *c)
{
	struct gsp_lite_r4p0_core *core = NULL;
...
int gsp_lite_r4p0_core_copy_cfg(struct gsp_kcfg *kcfg,
			void *arg, int index)
{
	struct gsp_lite_r4p0_cfg_user *cfg_user_arr = NULL;
	struct gsp_lite_r4p0_cfg_user *cfg_user;
	struct gsp_lite_r4p0_cfg *cfg = NULL;
	int icnt = 0;

	gsp_lite_r4p0_layer_num = 0;
	...
	memcpy(&cfg->ld1.params, &cfg_user->ld1.params,
		   sizeof(struct gsp_lite_r4p0_des_layer_params));
	gsp_layer_set_filled(&cfg->ld1.common);

	memcpy(&cfg->misc, &cfg_user->misc,
		   sizeof(struct gsp_lite_r4p0_misc_cfg));

	gsp_lite_r4p0_core_cfg_print(cfg);

	return 0;

}

int __user *gsp_lite_r4p0_core_intercept(void __user *arg, int index)
{
	struct gsp_lite_r4p0_cfg_user  __user *cfgs = NULL;

	cfgs = (struct gsp_lite_r4p0_cfg_user __user *)arg;

	return &cfgs[index].ld1.common.sig_fd;
}

void gsp_lite_r4p0_core_reset(struct gsp_core *core)
{
